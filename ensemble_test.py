...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset',
                        required=True,
                        choices={'ntu/xsub', 'ntu/xview', 'ntu120/xsub', 'ntu120/xset', 'NW-UCLA', 'gym'},
    ...
    if 'UCLA' in arg.dataset:
        ...
        if 'xsub' in arg.dataset:
            npz_data = np.load('../Hyper-GCN/data/' + 'ntu120/' + 'NTU120_CSub.npz')
        ...
        if 'xsub' in arg.dataset:
            ...
            label = np.where(npz_data['y_test'] > 0)[1]
    elif 'gym' in arg.dataset:
        label = []
        arg.alpha = [0.7, 0.5, 0.6, 0.2]
        with open('./data/' + 'gym_val.pkl', 'rb') as f:
            data_info = pickle.load(f)
            for index in range(len(data_info)):
                info = data_info[index]
    ...
    if arg.joint_motion_dir is not None and arg.bone_motion_dir is not None:
        ...
        space = [(0.1, 2.0) for i in range(2)]
